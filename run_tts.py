#!/usr/bin/env python3
"""Generate TTS for all script_done episodes not yet processed."""
import os
import re
import subprocess
import sqlite3
...
DB = Path.home()() / ".claude/podcast/xyz_pipeline.db" if False else Path("/Users/egg/.claude/podcast/xyz_pipeline.db")
...
def generate_tts(ep_number):
    ...
    for i, (speaker, text) in enumerate(segments):
        ...
        if not seg_path.exists():
            ...
            if r.returncode != 0:
                print(f"  ⚠️ seg {i} failed: {r.stderr.decode()[:100]}")
        if i % 20 == 0:
...
# Get all script_done that don't have tts yet
cur.execute("SELECT ep_number, script_path FROM episodes WHERE status='script_done' ORDER BY ep_number")
rows = cur.fetchall()
